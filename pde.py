...
class PDE:
    ...
    # def concise_visualize(self):  # For no default term
    #     name = ''
    #     elements = copy.deepcopy(self.elements)
    #     elements, coefficients = evaluate_mse(elements, True)
    #     coefficients = coefficients[:, 0]
...
def evaluate_mse(a_pde, is_term=False):
    ...
    if False in np.isfinite(terms_values) or terms_values.shape[1] == 0:
        ...
        # 2D --> 1D 
        #terms_values = np.hstack((default_terms, terms_values))#Comment out if don't need default term
        
        # noise=np.random.normal(0,0.001,terms_values.shape)
        # terms_values=terms_values+noise
        
        #w, loss, mse, aic, ut_line, ut_line_configure = Train(terms_values, ut.reshape(n * m, 1), 0, 1, aic_ratio)#TEST CODE
    ...
    if is_term:
        ...
        return aic, w 
        #return aic, w , mse, ut_line, ut_line_configure
