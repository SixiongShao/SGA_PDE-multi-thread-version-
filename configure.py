...
def get_var(avg_u,relative_var):#get variance according to relative variance
    return (avg_u*relative_var)**2
...
if problem == 'Null':
    ...
    right_side_origin = 'right_side_origin = 0'
...
if problem == 'chafee-infante': # 301*200的新数据
    ...
    # dx = x[2] - x[1]
    # for i in range(1,2):
    #     a1 = (u[i+2]-2*u[i]+u[i-2])/(4*dx**2)
    #     a2 = (u[i+1]-2*u[i]+u[i-1])/(dx**2)
    #     print(a1)
    #     print(a2)
    # # Calculate the first and second spatial derivatives of u
