...
class SGA:  # 最外层
    ...
    def run(self, gen=100):
        for i in range(1, gen+1):
            ...
            #print(i, "generation. Cross over number: ", self.cross_num, "Best AIC: [", self.the_best()[1], "]", self.the_best()[0].visualize())
            #print("Before change: [Length of eqs: ", len(self.eqs), " ]")
            self.change(self.p_mute, self.p_rep)
            #print(i, "generation. Mutation number: ",self.mut_num, ". Replacement number: ",self.rep_num )
            #print("After change: [Length of eqs: ", len(self.eqs), " ]")
            ...
            # add the best PDE of pool 2 & 3 to pool 1
            ...
            # if i % 10 == 0:
            #     #terms = self.collect_and_rank_terms()
            #     # terms = self.collect_unlikely_terms()
            #     # self.revise(terms)
            #     self.delete_unlikely_terms()
    ...
    def change(self, p_mute=0.05, p_rep=0.3):
        ...
        for i in range(self.num):            
            ...
            if i < 1: #保留最好的1个样本，不进行change
                continue
            #print("Before Mutation: ", new_eqs[i].visualize())
            new_eqs[i].mutate(p_mute)
            #print("Mutation ", i, " Best AIC: ", self.the_best()[1])
            #print("Mutation ", i, ": ", new_eqs[i].visualize() ," Best AIC: [", self.the_best()[1], "]" , self.the_best()[0].visualize())
            self.mut_num += 1

            replace_or_not = np.random.choice([False, True], p=([1 - p_rep, p_rep]))
            if replace_or_not:
                new_eqs[i].replace()
                #print("Replacement ", self.rep_num + 1, " Best AIC: ", self.the_best()[1])
                #print("Replacement ", self.rep_num + 1, " Best AIC: [", self.the_best()[1], "]" , self.the_best()[0].visualize())
                self.rep_num += 1
            # 检测是否重复
    ...
    def change3(self): #change method for pool 3 (100% mutate and replace on the PDE in the pool)
        for i in range(self.num): 
            if self.eqs3[i].W == 0:
                self.eqs3[i] = PDE(4,5,0.5)
                #self.eqs3[i] = PDE(4,1,0.5)
            else:         
        ...
        for i, ix in enumerate(sorted_indices):
            ...
            #print(self.eqs3[i].visualize(), self.mses3[i])
        
        # self.mses3 = self.mses3[:self.num//2]
        # self.eqs3 = self.eqs3[:self.num//2]
        
        # for i in range(self.num//2): # 循环产生pde
        #     a_pde = PDE(4,5,0.5)
        #     a_err, a_w = evaluate_mse(a_pde)
        #     pde_lib.append(a_pde)
        #     err_lib.append((a_err, a_w))
    ...
    # def collect_unlikely_terms(self):
    #     unlikely_terms = []
    #     coef = []
    #     pdes = []
    #     aics = []
    ...
    #         for j in range(len(coefficients)):
    #             if np.abs(coefficients[j]) >= 1e-4:  # Ignore terms with large coefficients
    #                 continue

    #             term = elements[j]
    #             unlikely_terms.append(term.inorder)
    #             coef.append(coefficients[j])
    #             aics.append(aic)
    #             pdes.append(pde.visualize())
    #             pdes_coefs.append(coefficients)

    #     print("Unlikely terms (with coefficients less than 1e-4):")
    #     for term, coefficient, aic, pde, pde_coefs in zip(unlikely_terms, coef, aics, pdes, pdes_coefs):
    #         print(f"Term: {term}, Coefficient: {coefficient}, in pde (aic: {aic}): {pde} ###{pde_coefs}")

    #     return unlikely_terms
    def delete_unlikely_terms(self):
        for i in range(min(20, len(self.eqs))):  # Collect terms from the first 20 PDEs
            # if (i==0):
            #     continue
            pde = self.eqs[i]
            _, coefficients = evaluate_mse(pde)
            coefficients = coefficients[1:, 0] #not include the weight of default u term

            for j in range(len(coefficients)):
                if np.abs(coefficients[j]) >= 1e-5:  # Ignore terms with large coefficients
                    continue

                element_pre = pde.elements[j]
                pde.elements[j] = Tree(4, 0.5)
                print(f"Replaced term {element_pre.inorder} with {pde.elements[j].inorder} in PDE {i}")
            
            # Update the error and the library
            a_err, a_w = evaluate_mse(pde)
            self.mses[i] = a_err
            pde_lib.append(pde.visualize())
            err_lib.append((a_err, a_w))
        print("Revision complete.")


    def revise(self, terms):
        for i in range(min(20, len(self.eqs))):  # Only revise the first 20 PDEs
            pde = self.eqs[i]
            for j, element in enumerate(pde.elements):
                if element.inorder in terms:
                    element_pre = pde.elements[j]
                    pde.elements[j] = Tree(4, 0.5)
                    print(f"Replaced term {element_pre.inorder} with {pde.elements[j].inorder} in PDE {i}")
    ...
    def replenish(self):
        total_elements = sum(len(pde.elements) for pde in self.eqs[:20])
        if total_elements < 40:#30
            new_total_elements = 0
            for i in range(1,20): #skip the best PDE
                if len(self.eqs[i].elements) <= 2:
                    random_element = Tree(4, 0.5)
                    self.eqs[i].elements.append(random_element)
                    self.eqs[i].W += 1
                    new_total_elements += 1
                    print(f"Added a random element {random_element.inorder} to PDE {self.eqs[i].visualize()}")
                    a_err, a_w = evaluate_mse(self.eqs[i])
                    if self.eqs[i].visualize() not in pde_lib:
                        pde_lib.append(self.eqs[i].visualize())
                        err_lib.append((a_err, a_w))
                    self.mses[i] = a_err
            print(f"Replenish complete. Elements: {total_elements} ---> {total_elements + new_total_elements}")
        else:
            print(f"Total elements: {total_elements}. Replenish not needed.")
